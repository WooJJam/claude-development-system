...
- [Architecture & Convention Docs](#architecture--convention-docs)
...
Step 4. Reviewer      전문 리뷰어 병렬 실행 → Orchestrator 취합·판정
          ├─ Plan Compliance  승인 계획 준수 검증
          ├─ Logic & Edge     로직 정확성·경계값 검증
          ├─ Convention       코드 컨벤션 검증
          └─ Architecture     아키텍처 문서 기반 검증 (문서 있을 때만)
...
| **Reviewer** | 4개 전문 리뷰어 병렬 실행(Plan Compliance / Logic & Edge Case / Convention / Architecture) + Orchestrator 취합·중복 제거·재구현 최종 판정. 이슈를 CRITICAL(즉시) / MAJOR(배포 전) / MINOR(선택)로 분류 | ✗ |
...
└── plugins/
    └── claude-taskforce/                 배포되는 플러그인
        ...
        └── skills/
            ...
            │       └── spring-boot.md
            ├── reviewer/
            │   ├── plan/skill.md        계획 준수 검증
            │   ├── logic/skill.md       로직·경계값 검증
            │   ├── convention/skill.md  코드 컨벤션 검증
            │   ├── architecture/skill.md 아키텍처 규칙 검증 (조건부)
            │   └── orchestrator/skill.md 취합·판정
            ├── qa/
            │   ├── skill.md
            │   └── references/
            │       └── spring-boot.md
...
## Architecture & Convention Docs

> **중요**: 아키텍처·컨벤션 문서는 반드시 **`docs/` 디렉터리 하위**에 위치해야 합니다. 다른 경로에 있는 파일은 리뷰어가 인식하지 않습니다.

### Architecture Review

프로젝트의 아키텍처 규칙(레이어 의존성, 금지 패턴 등)을 문서화하면, Reviewer가 구현이 해당 규칙을 준수하는지 자동으로 검증합니다.

**파일 위치**: `docs/` 하위, 파일명에 `arch`, `architecture`, `design`, `adr`, `structure` 중 하나 포함
...
docs/adr/                ✅
src/architecture.md      ❌ (docs/ 외부)
ARCHITECTURE.md          ❌ (docs/ 외부)
```

### Convention Review

프로젝트의 코딩 컨벤션(네이밍 규칙, 어노테이션 패턴, 패키지 구조 등)을 문서화하면, Reviewer가 일관성을 자동으로 검증합니다.

**파일 위치**: `docs/` 하위, 파일명에 `convention`, `style`, `coding`, `standard`, `guide` 중 하나 포함

```
docs/conventions.md      ✅
docs/coding-style.md     ✅
docs/dev-guide.md        ✅
CONTRIBUTING.md          ❌ (docs/ 외부)
```

### 문서가 없는 경우

- **아키텍처 문서 없음**: Architecture Reviewer가 실행되지 않습니다.
- **컨벤션 문서 없음**: Convention Reviewer가 기존 코드베이스 패턴을 추론하여 일관성을 검증합니다.
